...
function createbtn(category){
  let btn = `<button class= "btn-item btn btn-outline-dark" id="${category}">${category}</button>`;
  return btn;
}
...
function addBtn(){
  ...
  document.querySelector("#Korea").addEventListener("click",allKorea);
  document.querySelector("#Japan").addEventListener("click",allJapan);
  document.querySelector("#China").addEventListener("click",allChina);
}

addBtn();

function foodDisplay (menu){
  let items=
  `
